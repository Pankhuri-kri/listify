...
router.post('/login',async (req,res)=>{
    try{
        ...
        if(user.status !== "active") throw new Error("User's account is blocked, please contact to administrator");
        const validPassword = await bcrypt.compare(req.body.password, user.password);
        if(!validPassword) throw new Error('Invalid password');
        const token = jwt.sign({userId:user._id, email:user.email}, process.env.TOKEN_SECRET);
        res.send({ success:true, message:"user login in successfully", data:token });
    }catch(error){
        res.send({ success:false, message:error.message });
    }
});
...
router.get('/get-users', authMiddlewares, async (req,res)=>{
    try {
        ...
        res.send({ success:true, message:"user fetch succesfully", data:users });
    } catch (error) {
        res.send({ success:false, message:error.message });
    }
});
...
// Make any user an admin — for initial setup
router.put('/make-admin/:id', authMiddlewares, async (req, res) => {
    try {
